...
- `installPath` は絶対パスかつ `~/.claude/plugins/` 配下であることを `realpath` 解決後に検証する。範囲外のものは無視する
- シンボリックリンクは追う。`~/.claude/skills/*` は実際に `~/.agents/skills/*` へのリンクであり、追わないと機能しないため。ただしリンク先で読むのは対象の `.md` / `SKILL.md` のみとする
...
組み込みコマンドの固定リストは Claude Code のバージョンアップで古くなる。実用頻度の高いものだけに絞り、コードにその旨を明記する。
...
- セッションを切り替えると、そのセッションのプロジェクトに応じた一覧に更新される
