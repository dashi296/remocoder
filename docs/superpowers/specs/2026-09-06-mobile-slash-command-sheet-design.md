...
#### 脅威モデル（シンボリックリンクを追うことについて）

このスキャナはインストール済みプラグインの `plugin.json` に記載された `commands` / `skills` のパスも辿る。プラグインをインストールする操作自体が、このスキャナが持つ以上の実行能力（任意コード実行を含む）をそのプラグインに与えているため、**インストール済みプラグインは信頼境界の内側にある**。したがって、このスキャナの安全対策はプラグインを「悪意ある入力元」として遮断するためのものではない。

一方で、シンボリックリンクを追うことは意図的な仕様であり必須である。`~/.claude/skills/*` は実際に `~/.claude/` の外（`~/.agents/`）へのリンクであり、追わなければユーザー自身のスキルが一覧に出てこない。そのため、悪意ある `plugin.json` が `commands` / `skills` に外部ツリー（例: 任意のホームディレクトリ配下）へのシンボリックリンクを指定すれば、そのツリー配下にある `.md` / `SKILL.md` というファイル名と、それぞれの先頭 8KB の内容を列挙できてしまう。**上記の「リンク解決前のパスが期待するルート配下にあること」という検証は、こうした列挙を防ぐ境界にはならない。** それは名前の付け間違いや誤設定を弾くためのものであり、悪意あるプラグインに対する脱出防止策ではない。

実際にこの列挙範囲を絞っているのは以下の3点のみである。

- 読み取り対象を `.md` / `SKILL.md` に限定していること（他の拡張子・バイナリは読まない）
- 1ファイルあたり先頭 8KB のみを読むこと（ファイル全体は読まない）
- 走査の深度・ファイル数・時間の budget（深度3、500ファイル、3秒）

つまり、この設計はプラグインという既に信頼された存在からの意図的でない情報漏洩の面積を抑えるものであり、悪意あるプラグインを想定した隔離境界ではない。悪意あるプラグインへの対策は、このスキャナではなくプラグインのインストール・有効化フロー側の責務とする。
...
CTRL トグルの隣にコマンドボタンを追加する。`source?.kind === 'claude'` のときだけ表示する。props に `source` と `onOpenCommands` を追加する。

ラベルは `/…` とする。`BASE_KEYS` に既にリテラルの `/` キーがあるため、`/` にすると画面上で見分けがつかない。
