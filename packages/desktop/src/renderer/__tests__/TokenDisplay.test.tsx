...
describe('TokenDisplay', () => {
  describe('初期表示（マスク状態）', () => {
    ...
    it('表示ボタン（EyeIcon）が存在する', () => {
      render(<TokenDisplay token={TOKEN} />)
      expect(screen.getByTitle('表示')).toBeInTheDocument()
    })
  })
  ...
  describe('表示 / 非表示トグル', () => {
    it('表示ボタンをクリックするとトークン全体が表示される', async () => {
      ...
      render(<TokenDisplay token={TOKEN} />)
      await user.click(screen.getByTitle('表示'))
    ...
    it('表示後に隠すボタンが出現する', async () => {
      ...
      render(<TokenDisplay token={TOKEN} />)
      await user.click(screen.getByTitle('表示'))
      expect(screen.getByTitle('隠す')).toBeInTheDocument()
    })
    ...
    it('隠すボタンをクリックするとマスク表示に戻る', async () => {
      ...
      render(<TokenDisplay token={TOKEN} />)
      await user.click(screen.getByTitle('表示'))
      await user.click(screen.getByTitle('隠す'))
      expect(screen.queryByText(TOKEN)).not.toBeInTheDocument()
    })
  })
  ...
  describe('クリップボードコピー', () => {
    ...
    it('コピー後に "COPIED TO CLIPBOARD" バナーが表示される', async () => {
      ...
      const user = userEvent.setup()
      render(<TokenDisplay token={TOKEN} />)
      await user.click(screen.getByTitle('クリップボードにコピー'))
    ...
    it('コピー後にコピーボタンのアイコンが変わる（CheckIcon）', async () => {
      const user = userEvent.setup()
      render(<TokenDisplay token={TOKEN} />)
      const btn = screen.getByTitle('クリップボードにコピー')
